# Hosts 매핑 설계 (환경 분리 · 엔터프라이즈 도메인)

룰/스킬 본문과 MCP 설정에 박힌 weegloo URL들을 **origin 치환 테이블(hosts 매핑)** 로
...
| 적용 위치 | **인스톨러 측** (install + `--update` 공용 함수) | **콘텐츠 측 엔터프라이즈 브랜치(CI 생성) 기각** — 릴리스마다 N개 브랜치 재생성 운영비 + 백엔드가 브랜치를 알아야 함. 단, 완전 격리 납품 모델에서는 이 방식(hosts 파일을 CI에서 적용해 브랜치 생성)을 **이 구현 위에** 얹을 수 있음 — 상호 배타가 아님 |
| 매핑 단위 | 키는 **origin** (`https://<host>`, 검증용) — 치환은 **호스트 문자열 단위** | 경로 단위 매핑은 과설계. **호스트 단위 치환의 근거(실측)**: 본문에 scheme 없는 bare 호스트 언급이 52곳(auth 29·cma 17·cda 4·upload 2) — origin 단위로만 치환하면 산문 언급이 프로덕션 호스트로 남아 안내가 뒤섞임. 호스트 치환은 scheme URL(경로 보존)과 bare 언급을 한 번에 처리. 8개 호스트는 상호 비중첩이고 오답 예시(`cda-weegloo.com`)와도 문자 단위로 달라 안전 |
...
| 약관 게이트 | **`cma` 매핑 ⇒ `weegloo-terms-consent` 룰 자동 제외** | 게이트가 `cma…/v1/policy/terms` + `weegloo.com/terms`에 결합 — 다른 스택이면 약관 자체가 다르거나 계약으로 갈음. 링크(`weegloo.com/terms`)는 이 룰에만 존재(3곳)해 룰 제외와 함께 자연 소멸 |
| docs | **공용 고정** (매핑 제외) | 고객이 docs 미러를 둘 가능성 낮음. 폐쇄망 요구가 실제로 오면 그때 매핑 키 추가(순수 추가) |
| 콘텐츠 소스 | **공용 레포 단일** — 룰/스킬의 사설 레포(포크) 납품은 **비지원** | 엔터프라이즈도 콘텐츠는 공용, 도메인만 hosts 로 교체하는 모델. 덕분에 update 커맨드가 환경 무관하게 최소형으로 유지됨(repo 를 기록에 영속할 필요 없음). `WEEGLOO_REPO` env 는 개발/테스트용 오버라이드일 뿐 납품 경로 아님 |
...
```jsonc
// hosts.json — 값은 환경/고객별. 키는 아래 8개만 유효(모르는 키 → 에러). 부분 매핑 허용.
{
  "https://cma.weegloo.com":     "https://cma.acme.com",    // ⇒ terms-consent 룰 제외 (§6)
  "https://cda.weegloo.com":     "https://cda.acme.com",
  "https://acma.weegloo.com":    "https://acma.acme.com",
  "https://acda.weegloo.com":    "https://acda.acme.com",
  "https://upload.weegloo.com":  "https://upload.acme.com", // manifest.mcp.uploadApiUrl 에도 적용
  "https://auth.weegloo.com":    "https://auth.acme.com",   // 최대 표면(~50곳, provider redirect URI 포함)
  "https://console.weegloo.com": "https://console.acme.com",// PAT 페이지 + FE 로그인 팝업 origin
  "https://ai.weegloo.com":      "https://ai.acme.com"      // /v1/version(버전체크) + /mcp(MCP 서버)
}
```
...
**입력**: `--hosts <file>` 플래그 (또는 `WEEGLOO_HOSTS` env에 JSON/파일경로).
...
1. **콘텐츠 재작성**: `loadResources()` 결과의 skill 파일들 + 룰 content에
   `applyHostMapping(content, hosts)` (호스트 문자열 단위 replaceAll — §2 매핑 단위 참조) —
...
{ "skills": [...], "rules": [...], "availableSkills": [...], "availableRules": [...],
  "hosts": { "https://cma.weegloo.com": "https://cma.acme.com", ... } }
```

- `--update` 는 기록의 `hosts` 를 읽어 **자동 재적용** — 안 하면 업데이트가 콘텐츠를
  프로덕션 URL로 되돌리는 사고(이 설계에서 가장 위험한 실수 지점).
- **`--update` 는 `--hosts` 를 받지 않는다(조합 시 에러)** — update의 hosts 소스는
  ...
  제거** — 환경 전환(매핑 추가/변경/제거)은 전부 **재설치**(`--hosts` + 필요시 `--mcp`)가
  단일 경로. 에러 메시지가 이를 안내.
- 기록에 `hosts` 없음(기존 설치) = 매핑 없음 = 현행 동작.
- 안내 커맨드는 최소형 유지 — 브랜치(`ref`)와 같은 원리로 기록이 단일 소스.

## 6. 특수 규칙 — `cma` 매핑 ⇒ terms-consent 룰 제외

- `hosts` 에 `cma.weegloo.com` 키가 있으면 `weegloo-terms-consent` 를
...
### PR-A — 매핑 코어 (~1.5일)
- `hosts.js` (신규): 스키마 검증(유효 키 8개 · origin 형태 · 순환 거부) +
  `applyHostMapping(content, hosts)` + 매니페스트 mcp 적용 헬퍼.
- `cli.js`: `--hosts <file>` (+ `WEEGLOO_HOSTS` env) — **install 전용**, `--update` 와 조합 시 에러(§5).
- `index.js` / `update.js`: `loadResources` 후 콘텐츠·mcp 재작성 (룰은 템플릿 치환 **후**).
- 기록 스키마에 `hosts` 추가 (`self-update.js` read/write) + update 의 자동 재적용.

### PR-B — 특수 규칙 + 충돌 확장 (~1일)
- cma 매핑 ⇒ terms-consent 카탈로그 제거 + 코어 강제 조건부 (install·update 공통).
- 공유 스토어 충돌 감지에 hosts 상이 추가.

### 테스트
매핑 검증(잘못된 키/형태/순환) · 치환 결과(8 origin, 오답 예시 불변, 부분 매핑) ·
mcp URL 매핑 · 버전 룰 굽기 순서(`?branch=` + 매핑 동시) · 기록 영속 + update 재적용 ·
`--update --hosts` 조합 에러 · cma⇒terms 제외(체크박스/코어/update prune) · 공유 스토어 hosts 충돌 ·
매핑 없음 = 바이트 동일(회귀).
...
- **엔터프라이즈 브랜치(콘텐츠 측 CI)**: 운영비·백엔드 결합 — 단 이 구현 위의 배포
  방식으로는 여전히 유효 (hosts 파일을 CI 에서 적용해 고객 브랜치 생성).
