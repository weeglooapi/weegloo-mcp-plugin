...
4. On **write** (**`localized: true`**): the locale-presence requirement comes from the field's **`required`** flag, **not** from the field being populated. A **`required`** field must carry a value under **every non-optional locale** the space defines (the **default** is always non-optional, so always among them) — "just the default" is **insufficient** when another non-optional locale exists (`WGL400006`). A **non-`required`** field has **no** locale-presence requirement: `{ "ko-KR": "…" }` with no default entry is accepted.
