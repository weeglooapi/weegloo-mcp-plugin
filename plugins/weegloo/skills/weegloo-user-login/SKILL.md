...
description: Weegloo User login — authenticate an existing Weegloo platform account (Space owner or invited member) so the caller can call CMA, Upload and CDA as an admin. Not a sign-up flow and not the product's end-user signup (that is `weegloo-service-login`, scoped to ACMA/ACDA/Upload). Two mechanisms: a Personal Access Token (PAT) for server / CI / scripts, and the Weegloo Console FE login popup for browser apps, including static sites on Weegloo WebHosting. Use when building an API-driven custom admin UI, a Weegloo-User-only internal/staff product, or CMA-authenticated editing from a static site. 관리자 로그인, 콘솔 로그인, 어드민 화면, 스태프 전용, 내부용.
---
