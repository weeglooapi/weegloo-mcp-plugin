...
- **레거시 공유 경로**(`~/.weegloo/*.json`)에는 **더 이상 쓰지 않음**(파일은 방치).
  이유: 새 설치가 레거시 스탬프를 계속 갱신하면 **미마이그레이션 에이전트의 옛 룰이 그걸 읽고
  "최신"으로 오판**해 영영 노티를 못 받음. 쓰기를 끊으면 옛 룰이 stale 스탬프를 보고 정상적으로
  업데이트를 촉구 → 자연 마이그레이션.
- **단, 레거시 `installed.json`은 1회성 read-fallback** — 설치의 정리(removal) diff가 per-agent
  기록을 읽는데, 마이그레이션 설치 시점엔 그 파일이 아직 없어 prev가 빈 목록이 됨 → upstream에서
  삭제된 skill/rule이 **디스크에 고아로 영영 남는 구멍**. per-agent 기록 부재 시 레거시 flat
  기록을 prev로 읽으면 닫힘. 안전한 이유: 정리 콜백은 그 에이전트의 디렉터리에서 **존재하는
  것만** 지우므로(existence-check, `io.js`), flat 기록에 섞인 다른 에이전트의 id는 no-op.
  레거시 **스탬프**는 읽지 않음 — `ref`도 없고 `version`은 어차피 브랜치 매니페스트로 재기록.
...
- **레거시 공유 stamp/record**: 쓰기 중단 + **방치** (삭제 안 함). 조기 삭제 불가 이유: flat
  `installed.json`은 각 에이전트 첫 마이그레이션의 정리-diff fallback이라 먼저 넘어온 에이전트가
  지우면 나중 에이전트에 고아(stale 컨텍스트로 계속 로드됨)가 재발하고, flat 스탬프는 옛 룰이
  재생성. **조건부 삭제("전원 마이그레이션 완료 시 마지막이 불 끄기")는 검토 후 기각** — 판정
  휴리스틱(에이전트 5종 × skills/rules/마커) 비용 대비 얻는 게 inert JSON 2개(~200B) 삭제뿐이고
  오판 시 실해. 필요해지면 추후 릴리스에 순수 추가 가능(설계 변경 불요).
