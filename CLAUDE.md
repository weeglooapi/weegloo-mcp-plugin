...
1. **단일 소유자.** 하나의 사실에 하나의 권위 파일. 다른 곳의 언급은 그것에 **동의하고 가리키는**
   한 줄(Verdict)이어야 한다.
2. **부모 → 자식 링크와 형제 스킬 링크 둘 다 합법.** 스킬이 다른 스킬을 가리켜도 되고, 그 스킬의
   `references/` 안 특정 파일까지 가리켜도 된다. 대신 **끊어질 때 조용하지 않도록** 쓴다:
   - **소유 스킬을 먼저 이름으로 밝힌다.** `weegloo-service-login-client` → 그 스킬의
     `references/google.md` 처럼. 경로만 적힌 링크는 파일이 옮겨가면 되찾을 단서가 없다.
   - **§1.3-4는 그대로 적용된다.** 링크를 단 줄 자체가 링크를 열지 않고도 행동 가능해야 한다.
     형제 링크도 정밀도를 사지, 정확성을 사지 않는다.
   - 링크는 **소유권을 옮기지 않는다**(§1.3-1). 가리키는 쪽은 여전히 동의하는 Verdict 한 줄이고,
     같은 내용을 복제해야 한다면 `FACT-OWNERS.md`에 등록한다.

   **아는 대가:** 대상 스킬이 그 reference를 이름 바꾸거나 쪼개면 포인터는 **아무 에러 없이** 낡는다
   — 이 저장소가 결함이라 부르는 바로 그 형태다. 지금 크로스-스킬 링크 대상을 검사하는 테스트는
   없으므로, 이 제약을 대신 지켜 주는 것은 **§1.3-4 하나뿐**이다. 그래서 Verdict 없이 링크만 남기는
   줄은 여전히 금지다.

   > 이 항목은 원래 금지였다(형제 `references/` 링크 불가). 근거는 결합이 깊어진다는 것이었는데,
   > 4번과 달리 **회귀 사례로 뒷받침된 적이 없고**, 코퍼스에는 그 상태로 fixtures를 통과해 온 형제
   > 링크가 이미 셋 있었다(`weegloo-platform-integration`·`weegloo-service-login` → 
   > `weegloo-service-login-client`의 `references/{provider}.md`). 반대로 금지는 실제 비용을 냈다 —
   > `weegloo-frontend` 분리에서 `git mv` 하나가 멀쩡하던 부모→자식 링크 둘을 위반으로 바꿨고,
   > 정밀한 포인터를 잃게 만들었다.
3. **깊이 ≤ 3** (§1.1).
...
- 형제 스킬의 `references/`를 가리켜도 된다(§1.3-2) — **소유 스킬을 먼저 이름으로** 밝히고, 그 줄이
  링크 없이도 행동 가능해야 한다.
