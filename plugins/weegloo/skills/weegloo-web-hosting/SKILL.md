...
- **MarketApp packaging — the server auto-substitutes IDs/tokens at install time.** If this WebHosting may *ever* be packaged into a MarketApp later, you need to know up front that **the Weegloo server automatically rewrites the source Space's `sys.id`s and `DeliveryAccessToken` strings into the installing Space's values at install time**. Without knowing that mechanism exists, an agent has no reason to inline the source Space's literal values into the build output — it will invent placeholder syntax, hide tokens behind env-var indirection, or leave the values blank as "the author's Space shouldn't be hard-coded", and all of those silently break because the rewrite only matches **contiguous literal occurrences in text files**. Read **`weegloo-marketapp-packaging`** **before** writing build-time config. Direct deploys (the workflow above) have no such mechanism.
