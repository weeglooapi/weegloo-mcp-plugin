...
description: Deleting a Weegloo resource fails while something still depends on it — Weegloo never force-deletes and never cascades upward. Covers Space teardown order (only ContentType, Content, Media and WebHosting block a Space), the ContentType↔Content dependency, the Draft-or-Archived precondition (unpublish first), busy files and in-flight deploys, and the preconditions for Locale, SpaceRole, ServiceUserRole, Script, Tag and Organization — plus dangling `Refer` stubs, and who may delete a Space or an Organization. Use when deleting or cleaning up ANY Weegloo resource, planning a teardown, or a delete returns 422. 삭제, 지우기, 정리, 초기화, 스페이스 삭제.
---
