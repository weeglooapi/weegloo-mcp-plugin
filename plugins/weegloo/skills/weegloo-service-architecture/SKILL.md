...
- **Content modeling for a forum/board (verified end-to-end):**
  - **Author:** set **`publishWithAuthor: true`** on the post/comment `ContentType`. The writing `ServiceUser` is preserved as **`sys.createdBy`** and delivered with the published snapshot on **ACDA / CDA** (resolve with `include=1` for the byline). Do **not** add a manual author field. Enable it **before** members start posting - it is applied at publish time and is **not** retroactive. This is not forum-specific: it applies to **any** member-generated `ContentType` whose author you will **display or filter by** (comments, reviews, profiles, uploads). With the default **`false`** the delivered snapshot has **no** `sys.createdBy`, so bylines and `:self` filters silently break on ACDA / CDA. See **`weegloo-service-login`** → *Author of member content* and **`weegloo-create-content-type`**.
