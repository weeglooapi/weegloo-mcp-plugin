...
- **Content modeling for a forum/board (verified end-to-end):**
  - **Author:** set **`publishWithAuthor: true`** on any member-generated `ContentType` whose author you will **display or filter by** (posts, comments, reviews, profiles). `sys.createdBy` is then delivered with the snapshot on **ACDA / CDA** (`include=1` for the byline); do **not** add a manual author field. It is `false` by default and not retroactive, so set it **before** members post. Mechanism: **`weegloo-create-content-type`** → *`publishWithAuthor`*.
