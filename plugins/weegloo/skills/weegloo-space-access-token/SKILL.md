...
description: Create a Weegloo SpaceAccessToken (CMA) — a read+write token confined to ONE Space whose power is set entirely by a bound SpaceRole. Use when the user asks for a "Space token", a scoped API key that can write to this Space, an anonymous-write / public-submit token, or a credential narrower than a Personal Access Token but write-capable, unlike a read-only DeliveryAccessToken: 스페이스 토큰, 쓰기 가능한 토큰, 익명 글쓰기, 토큰 발급, 권한 제한. Covers what it authorizes (CMA data + CDA + Upload), what it never reaches (Organization, account plane, ACMA/ACDA, every SETTING_*), binding the role, one exposed key vs two, and rotation.
---
